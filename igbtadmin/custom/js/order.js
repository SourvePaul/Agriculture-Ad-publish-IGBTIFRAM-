...
$(document).ready(function () {
                        ...
                        success: function (response) {
                            ...
                            if (response.success == true) {
                                ...
                                $(".success-messages").html('<div class="alert alert-success">' +
                                    ...
                                    ' <br /> <br /> <a href="singleSalesInvoice.php?id=' + response.order_id + '" class="btn btn-primary"> <i class="glyphicon glyphicon-info-sign"></i> View </a>' +
                        ...
                        success: function (response) {
                            ...
                            if (response.success == true) {
                                ...
                                $(".success-messages").html('<div class="alert alert-success">' +
                                    '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> ' + response.messages +
                                    ' <br /> <br /> <a href="singleMMRview.php?id=' + response.mmr_id + '" class="btn btn-primary"> <i class="glyphicon glyphicon-info-sign"></i> View </a>' +
                                    '<a href="orders.php?o=iss" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New </a>' +
                                    '</div>');
                        ...
                        success: function (response) {
                            ...
                            if (response.success == true) {
                                ...
                                $(".success-messages").html('<div class="alert alert-success">' +
                                    ...
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> ' + response.messages +
                                    ' <br /> <br /> <a href="singleMMRview.php?id=' + response.mmr_id + '" class="btn btn-primary"> <i class="glyphicon glyphicon-info-sign"></i> View </a>' +
                                    '<a href="orders.php?o=rcv" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New </a>' +
                                    '</div>');
    ...
    else if (divRequest == 'mmr') {
        // top nav child bar
        $('#topNavManageOrder').addClass('active');

        manageOrderTable = $("#manageOrderTable").DataTable({
            'ajax': 'php_action/fetchMMR.php',
            'order': []
        });
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var tr = '<tr id="row' + count + '" class="' + arrayNumber + '">' +
                '<td style="margin-left:20px; padding-right: 20px">' +
            ...
            tr += '</select>' +
                ...
                '<div class="form-group">' +
                '<input type="number" id="quantity2' + count + '" onkeyup="getTotal(' + count + ')" autocomplete="off" class="form-control" min="1" />' +
                '</div>' +
                '</td>' +
                '<td style="padding-left:20px;"">' +
                '<input type="text" name="rate[]" id="rate' + count + '" onkeyup="getTotal(' + count + ')" onchange="getTotal(' + count + ')" autocomplete="off"  class="form-control" />' +
...
function addRowForRcv() {
    $("#addRowBtnForRcv").button("loading");
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            tr += '</select>' +
                ...
                '<select class="form-control" name="wareHouse[]" id="wareHouse' + count + '" autocomplete="off" required>' +
                '<option value="">~~SELECT~~</option>'+
        ...
        if (productId == "") {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#quantity" + row).val(1);
                    $("#quantity2" + row).val(response.quantity);
...
//get clint Detail
function getClintDetail(row = null) {

    if (row) {
        var clintID = $("#clientName").val();
        if (clintID == 15) {
            $("#clientContact").val();
            $("#clientAddress").val();
            document.getElementById("nonListedClassName").style.display = "inline";
            $("#nonListedName").val();
        } else {
           // console.log(clintID);
            $.ajax({
                ...
                success: function (response) {
                    //console.log(response);
                    document.getElementById("nonListedClassName").style.display = "none";
                    $("#clientContact").val(response.cell);
